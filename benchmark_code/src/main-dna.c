...
void print_help() {
    printf("Usage: ./libsais64_dna <input_file> <output_file>\n");
}
...
int main(int argc, char *argv[]) {
    ...
    size_t packed_len = 0;
    int64_t sparseness_factor = 4;
    uint8_t* packed_text = bitpack_text_8(text, length, sparseness_factor, &packed_len, 1);
    free(text);
    ...
    libsais64(packed_text, sa, packed_len, 0, NULL);
